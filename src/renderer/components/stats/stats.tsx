...
const getRainPeriods = (input: dataItem[], property: propertyParameter) => {
  const dataBundledPerDay = bundleData(input, property, "day"),
    data = Object.values(dataBundledPerDay)
      .slice()
      .filter((item) => Math.max(...item.values) >= 0.1)
  ...
  for (let i = 0; i < data.length; i++) {
    if (i === 0) {
      ...
      for (let k = 0; k < splitIntoSeries.length; k++) {
        if (
          moment
            .unix(splitIntoSeries[k][splitIntoSeries[k].length - 1].time)
            .utc()
            .add(1, "days")
            .format("DDMMYYYY") ===
          moment.unix(data[i].time).utc().format("DDMMYYYY")
        ) {
      ...
      if (!added) {
        splitIntoSeries = [...splitIntoSeries, [data[i]]];
      }
    }
  }
  ...
  for (const statsKey of props.stats) {
    if (dataHasRecordsForProperty(statsKey.property, props.data)) {
      ...
      switch (statsKey.direction) {
            ...
            switch (statsKey.scaling) {
              case "sum": {
                const data = scale(
                  ...
                  statsKey.direction === "max"
                    ? b[statsKey.property] - a[statsKey.property]
                    : a[statsKey.property] - b[statsKey.property]
                );

                date = moment(data[0].timeParsed).format(
                  statsKey.dateFormat ? statsKey.dateFormat : "YYYY/MM/DD"
                );
        ...
        case "day": {
          const dataBundledPerDay = bundleData(
            props.data,
            statsKey.property,
            "day"
          );
          ...
          switch (statsKey.extra) {
            case "storm-days": {
              const data = Object.values(dataBundledPerDay)
                .slice()
                .filter((item) => Math.max(...item.values) >= 62).length;
            ...
            case "rain-days": {
              const data = Object.values(dataBundledPerDay)
                .slice()
                .filter((item) => Math.max(...item.values) >= 0.1).length;
            ...
            case "frost-days": {
              const data = Object.values(dataBundledPerDay)
                .slice()
                .filter((item) => Math.min(...item.values) < 0).length;
            ...
            case "ice-days": {
              const data = Object.values(dataBundledPerDay)
                .slice()
                .filter((item) => Math.max(...item.values) < 0).length;
            ...
            case "summer-days": {
              const data = Object.values(dataBundledPerDay)
                .slice()
                .filter((item) => Math.max(...item.values) >= 25).length;
            ...
            case "hot-days": {
              const data = Object.values(dataBundledPerDay)
                .slice()
                .filter((item) => Math.max(...item.values) >= 30).length;
            ...
            case "tropical-nights": {
              const dataBundledPerDay = bundleData(
                  props.data.filter(
                    (item) =>
                      parseInt(moment.unix(item.time).utc().format("HH")) >=
                        18 ||
                      parseInt(moment.unix(item.time).utc().format("HH")) < 6
                  ),
                  statsKey.property,
                  "day"
                ),
                data = Object.values(dataBundledPerDay)
                  .slice()
                  .filter((item) => Math.min(...item.values) >= 20).length;
            ...
            case "desert-days": {
              const data = Object.values(dataBundledPerDay)
                .slice()
                .filter((item) => Math.max(...item.values) >= 35).length;
        ...
        case "extra": {
          switch (statsKey.extra) {
            case "min-max-diff-up": {
              const dataBundledPerDay = Object.values(
                bundleData(props.data, statsKey.property, "day")
              )
                .map((item) => {
                  const minIndex = item.values.lastIndexOf(
                      Math.min(...item.values)
                    ),
                    maxValue = Math.max(
                      ...item.values.slice(minIndex, item.values.length)
                    ),
              ...
              date = moment
                .unix(dataBundledPerDay[0].time)
                .utc()
                .format("YYYY/MM/DD");
              value = `+ ${dataBundledPerDay[0].rise.toFixed(1)} ${
                statsKey.unit
              }`;
              statsKey.tooltip = `Min: ${dataBundledPerDay[0].min.toFixed(1)} ${
                statsKey.unit
              }, Max: ${dataBundledPerDay[0].max.toFixed(1)} ${statsKey.unit}`;
              break;
            }
            case "min-max-diff-down": {
              const dataBundledPerDay = Object.values(
                ...
                .map((item) => {
                  const maxIndex = item.values.lastIndexOf(
                      Math.max(...item.values)
                    ),
                    minValue = Math.min(
                      ...item.values.slice(maxIndex, item.values.length)
                    ),
              ...
              date = moment
                ...
                .utc()
                .format("YYYY/MM/DD");
              value = `${dataBundledPerDay[0].fall.toFixed(1)} ${
                statsKey.unit
              }`;
              statsKey.tooltip = `Min: ${dataBundledPerDay[0].min.toFixed(1)} ${
                statsKey.unit
            ...
            case "rain-days-consecutive-sum": {
              ...
              statsKey.tooltip = `${moment
                .unix(periods[0][0].time)
                .utc()
                .format("YYYY/MM/DD")} - ${moment
                .unix(periods[0][periods[0].length - 1].time)
                .utc()
                .format("YYYY/MM/DD")}`;
              break;
            }
