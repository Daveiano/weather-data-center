...
afterAll(async () => {
  ...
  await fs.truncateSync(
    `${__dirname.replace("src/main", "")}tests/data/empty/nedb/data`,
    0
  );
});
...
describe("empty", () => {
  ...
  it("should start the app on an empty state", async () => {
    ...
    // Open right sidebar.
    await page.click(
      'header.bx--header button[aria-label="Database / Import"]'
    );

    // Expect 0 items to be imported.
    const numberImported = page.locator(
      "header.bx--header .bx--header-panel .import-data"
    );
  ...
  it("should import data with custom settings", async () => {
    ...
    await page.waitForSelector("data-testid=modal-loading", {
      state: "hidden",
    });
    ...
    await electronApp.evaluate(
      ...
      [
        `${__dirname.replace(
          "src/main",
          ""
        )}tests/data/upload-data-04-03-21-14-03-21-145-records.CSV`,
      ]
    );
    ...
    const temperature_stat = page.locator(
      ".tiles .stats .stat-tile:nth-child(1) .value"
    );
    expect(await temperature_stat.evaluate((node) => node.textContent)).toBe(
      "10.3 CC"
    );

    const wind_stat = page.locator(
      ".tiles .stats .stat-tile:nth-child(3) .value"
    );
    expect(await wind_stat.evaluate((node) => node.textContent)).toBe(
      "33.1 mp/h"
    );

    const rain_stat = page.locator(
      ".tiles .stats .stat-tile:nth-child(4) .value"
    );
    expect(await rain_stat.evaluate((node) => node.textContent)).toBe("5.4 l");

    const pressure_stat = page.locator(
      ".tiles .stats .stat-tile:nth-child(5) .value"
    );
    expect(await pressure_stat.evaluate((node) => node.textContent)).toBe(
      "1000.6 bar"
    );
  });
    ...
    await electronApp.evaluate(
      ...
      [
        `${__dirname.replace(
          "src/main",
          ""
        )}tests/data/upload-data-16-08-21-30-09-21-1196-records.csv`,
      ]
    );
...
it("should not import duplicates", async () => {
  ...
  const numberImportedAfterSecondImport = page.locator(
    "header.bx--header .bx--header-panel .import-data > div"
  );
  expect(
    await numberImportedAfterSecondImport.evaluate((node) => node.textContent)
  ).toBe("1196 records imported");

  const duplicateItems = page.locator(
    "header.bx--header .bx--header-panel .import-action .bx--inline-notification__subtitle"
  );
  expect(await duplicateItems.evaluate((node) => node.textContent)).toBe(
    "1196 items were duplicate and are not imported."
  );
});
  ...
  await electronApp.evaluate(
    ...
    [
      `${__dirname.replace(
        "src/main",
        ""
      )}tests/data/upload-data-01-10-21-15-10-21-1197-records.CSV`,
    ]
  );
...
it("should delete single records", async () => {
  ...
  expect(await tableRows.evaluate((node) => node.textContent)).toBe(
    "1–100 of 2393 items"
  );

  // Check 5 single records.
  await page.check(
    '.bx--data-table-content tbody label[aria-label="Select row"] >> nth=0'
  );
  await page.check(
    '.bx--data-table-content tbody label[aria-label="Select row"] >> nth=2'
  );
  await page.check(
    '.bx--data-table-content tbody label[aria-label="Select row"] >> nth=3'
  );
  await page.check(
    '.bx--data-table-content tbody label[aria-label="Select row"] >> nth=4'
  );
  await page.check(
    '.bx--data-table-content tbody label[aria-label="Select row"] >> nth=5'
  );

  await page.click(
    ".bx--batch-actions.bx--batch-actions--active div.bx--action-list button:nth-child(1)"
  );
  await page.waitForSelector(
    ".page .start-tables .bx--modal.bx--modal-tall.bx--modal--danger h3",
    { state: "visible" }
  );

  const modalText = page.locator(
    ".page .start-tables .bx--modal.bx--modal-tall.is-visible.bx--modal--danger h3.bx--modal-header__heading"
  );
  expect(await modalText.evaluate((node) => node.textContent)).toBe(
    "Are you sure? This will delete 5 record(s)."
  );
  ...
  await page.click(
    ".page .start-tables .bx--modal--danger button.bx--btn--danger"
  );
...
it("should delete all records", async () => {
  ...
  // Check all records.
  await page.check(
    '.bx--data-table-content thead label[aria-label="Select all rows"]'
  );
  ...
  expect(await modalText.evaluate((node) => node.textContent)).toBe(
    "Are you sure? This will delete 2388 record(s)."
  );
